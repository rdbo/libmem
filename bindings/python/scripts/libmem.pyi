...
def LM_DataScan(data : bytearray, addr : int, scansize : int) -> int: ...
def LM_DataScanEx(pproc : lm_process_t, data : bytearray, addr : int, scansize : int) -> int: ...
def LM_DemangleSymbol(symbol : str) -> str: ...
def LM_Disassemble(code : int) -> lm_inst_t: ...
def LM_DisassembleEx(code : int, bits : int, size : int, count : int, runtime_addr : int) -> lm_inst_t: ...
def LM_EnumModules() -> List[lm_module_t]: ...
def LM_EnumModulesEx(pproc : lm_process_t) -> List[lm_module_t]: ...
def LM_EnumPages() -> lm_page_t: ...
def LM_EnumProcesses() -> List[lm_process_t]: ...
def LM_EnumSymbols(pmod : lm_module_t) -> lm_symbol_t: ...
def LM_EnumSymbolsDemangled(pmod : lm_module_t) -> lm_symbol_t: ...
...
def LM_HookCode(_from: int, to : int) -> int: ...
def LM_HookCodeEx(pproc : lm_process_t, _from: int, to : int) -> int: ...
...
def LM_PatternScan(pattern : bytearray, mask : str, addr : int, scansize : int) -> int: ...
def LM_PatternScanEx(pproc : lm_process_t, pattern : bytearray, mask : str, addr : int, scansize : int) -> int: ...
def LM_ProtMemory(addr : int, size : int, prot : lm_prot_t) -> lm_prot_t: ...
def LM_ProtMemoryEx(pproc : lm_process_t, addr : int, size : int, prot : lm_prot_t) -> lm_prot_t: ...
def LM_ReadMemory(src : int, size : int) -> bytearray: ...
def LM_ReadMemoryEx(pproc : lm_process_t, src : int, size : int) -> bytearray: ...
def LM_SetMemory(dst : int, byte : bytes, size : int) -> bool: ...
def LM_SetMemoryEx(pproc : lm_process_t, dst : int, byte : bytes, size : int) -> bool: ...
def LM_SigScan(sig : str, addr : int, scansize : int) -> int: ...
def LM_SigScanEx(pproc : lm_process_t, sig : str, addr : int, scansize : int) -> int: ...
def LM_UnhookCode(_from: int, trampoline : (int, int)) -> bool: ...
def LM_UnhookCodeEx(pproc : lm_process_t, _from: int, trampoline : (int, int)) -> bool: ...
