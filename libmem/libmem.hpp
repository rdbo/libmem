...
namespace mem
{
	namespace in
	{
		...
		{
			mem_string_t  str = MEM_STR("");
			mem_tstring_t process_name = (mem_tstring_t)NULL;
			mem_size_t    read_chars = mem::in::get_process_name(&process_name);
			if (read_chars && process_name)
			{
				str = process_name;
				free(process_name);
			}
		...
		inline mem_string_t                 get_process_path(mem_void_t)
		{
			...
			mem_size_t    read_chars = mem::in::get_process_path(&process_path);
			if (read_chars && process_path)
			{
				str = process_path;
				free(process_path);
			}
		...
		inline mem_string_t                 get_module_name(mem_module_t mod)
		{
			...
			mem_size_t    read_chars = mem::in::get_module_name(mod, &module_name);
			if (read_chars && module_name)
			{
				str = module_name;
				free(module_name);
			}
		...
		inline mem_string_t                 get_module_path(mem_module_t mod)
		{
			...
			mem_size_t    read_chars = mem::in::get_module_path(mod, &module_path);
			if (read_chars && module_path)
			{
				str = module_path;
				free(module_path);
			}
		...
		inline std::vector<mem_module_t>    get_module_list(mem_void_t)
		{
			...
			mem_size_t size = mem::in::get_module_list(&module_list);
		...
		inline mem_voidptr_t                signature_scan(mem_tstring_t signature, mem_voidptr_t start, mem_voidptr_t stop) { return mem_in_signature_scan(signature, start, stop); }
		inline mem_voidptr_t                signature_scan(mem_string_t signature, mem_voidptr_t start, mem_voidptr_t stop) { return mem_in_signature_scan((mem_tstring_t)signature.c_str(), start, stop); }
		inline mem_voidptr_t                signature_scan(mem_string_t signature, mem_module_t mod) { return mem_in_signature_scan((mem_tstring_t)signature.c_str(), mod.base, mod.end); }
		inline mem_voidptr_t                signature_scan(mem_string_t signature, mem_page_t page) { return mem_in_signature_scan((mem_tstring_t)signature.c_str(), page.base, page.end); }
	...
	namespace ex
	{
		...
		inline mem_string_t                 get_process_name(mem_pid_t pid)
		{
			mem_string_t  str = MEM_STR("");
			mem_tstring_t process_name = (mem_tstring_t)NULL;
			mem_size_t    read_chars = mem::ex::get_process_name(pid, &process_name);
			if (read_chars && process_name)
			{
				str = process_name;
				free(process_name);
		...
		inline mem_string_t                 get_process_path(mem_pid_t pid)
		{
			mem_string_t  str = MEM_STR("");
			mem_tstring_t process_path = (mem_tstring_t)NULL;
			mem_size_t    read_chars = mem::ex::get_process_path(pid, &process_path);
			if (read_chars && process_path)
			{
				str = process_path;
				free(process_path);
		...
		inline std::vector<mem_process_t>   get_process_list(mem_void_t)
		{
			...
			mem_size_t size = mem::ex::get_process_list(&process_list);
			std::vector<mem_process_t> proc_list = {};
			if (size && process_list)
			{
				for (mem_size_t i = 0; i < size; ++i)
		...
		inline mem_string_t                 get_module_name(mem_process_t process, mem_module_t mod)
		{
			mem_string_t  str = MEM_STR("");
			mem_tstring_t module_name = (mem_tstring_t)NULL;
			mem_size_t    read_chars = mem::ex::get_module_name(process, mod, &module_name);
			if (read_chars && module_name)
			{
				str = module_name;
				free(module_name);
		...
		inline mem_string_t                 get_module_path(mem_process_t process, mem_module_t mod)
		{
			mem_string_t  str = MEM_STR("");
			mem_tstring_t module_path = (mem_tstring_t)NULL;
			mem_size_t    read_chars = mem::ex::get_module_path(process, mod, &module_path);
			if (read_chars && module_path)
			{
				str = module_path;
				free(module_path);
		...
		{
			mem_module_t* module_list = (mem_module_t*)NULL;
			mem_size_t size = mem::ex::get_module_list(process, &module_list);
			std::vector<mem_module_t> mod_list = {};
			if (size && module_list)
			{
				for (mem_size_t i = 0; i < size; ++i)
		...
		inline mem_voidptr_t                signature_scan(mem_process_t process, mem_tstring_t signature, mem_voidptr_t start, mem_voidptr_t stop) { return mem_ex_signature_scan(process, signature, start, stop); }
		inline mem_voidptr_t                signature_scan(mem_process_t process, mem_string_t signature, mem_voidptr_t start, mem_voidptr_t stop) { return mem_ex_signature_scan(process, (mem_tstring_t)signature.c_str(), start, stop); }
		inline mem_voidptr_t                signature_scan(mem_process_t process, mem_string_t signature, mem_module_t mod) { return mem_ex_signature_scan(process, (mem_tstring_t)signature.c_str(), mod.base, mod.end); }
		inline mem_voidptr_t                signature_scan(mem_process_t process, mem_string_t signature, mem_page_t page) { return mem_ex_signature_scan(process, (mem_tstring_t)signature.c_str(), page.base, page.end); }
